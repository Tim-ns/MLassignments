...
def validate_poly_regression(X_train, y_train, X_val, y_val,
    ...
    for deg in degrees:
        ...
        polyreg = make_pipeline(PolynomialFeatures(deg), StandardScaler(), regressor)
        val_rmse_scores = []
        train_rmse_scores = []
        ...
        for train_index, val_index in kf.split(X_train):
            ...
            polyreg.fit(X_train_fold, y_train_fold)

            # Predict validation and train
            y_pred_val = polyreg.predict(X_val_fold)
            y_pred_train = polyreg.predict(X_train_fold)

            # Calculate RMSE for validation and train sets
            rmse_val = math.sqrt(np.square(np.subtract(y_val_fold, y_pred_val)).mean())
            rmse_train = math.sqrt(np.square(np.subtract(y_train_fold, y_pred_train)).mean())

            val_rmse_scores.append(rmse_val)
            train_rmse_scores.append(rmse_train)

        # Average RMSE across the folds
        print(f'Degree {deg}: Validation RMSE = {np.mean(val_rmse_scores):.4f}, Training RMSE = {np.mean(train_rmse_scores):.4f}')
...
model, error = validate_poly_regression(X_train, y_train, X_val, y_val)
